...
**Ticket t2 · task 85baf1fc · 2026-09-10 · refreshed f3, f4, g3.** This document renders `docs/decision-graph.json` (the machine-readable graph is authoritative). Every load-bearing decision carries **authority** (constraint / ours / provider), **evidence** (tier: tested / configuration-tested / assumed; kind; source), and **status** (proposed / accepted / superseded). Per R87 and AC0, conflicts sit at the top. **f3 refresh (AC2 — "the graph updates itself as phases run"):** the four conflicts the verification review found undeclared (C21) are added as completed results at the top; the fix rounds' load-bearing decisions are recorded as nodes (§"Fix-round decisions"); statuses and counts corrected where fixes obsoleted the t2 text. **f4 addition (A-j):** the R58-vs-R19 door-price conflict (item 1 below) -- previously recorded only as decision node `D-PRICE-ARM-CHARGE`, now ALSO declared as the R87 conflict it actually resolves. **g3 refresh (round 2, CB-BUILD-009r + 014-022 -- AC2's "the graph updates itself as phases run" made good on again):** round 2 moved several load-bearing decisions the graph had not recorded (R81/CB-BUILD-009r reversed by owner ruling, O6's default flipped, a new sound-on-by-default LAW, a new summon-ceremony LAW, a corrected AC1 measurement start event, the read-time identity derivation plus its fix-round F3 carry restoration, and a retention-metric bug fix) -- all seven now recorded as nodes (§"Fix-round decisions (round 2 / g3)"); the one R87 tension round 2 actually produced (R74 vs the round-1 A-power rule) is declared as item 9 below; four round-2 ids reviewed and found NOT independently load-bearing are noted with reasoning rather than padded into full nodes (`NOTE-ROUND2-NON-LOAD-BEARING`); `O6-LOBBY`'s stale default corrected in place.

---

## ⚠ Conflicts (R87) — at the top, as required

**Were any found? Yes — nine.** Declared at t2: one law conflict, one evidence conflict, one data discrepancy (items 6–8 below). Added at f3 (found undeclared by the verification review, C21): three law conflicts and one internal-rule conflict, each **delivered as a completed result** (items 2–5). Added at f4 (re-probe v1b/A-j): one law conflict (R58's literal accounts arithmetic vs R19's door-price law), delivered as a completed result (item 1). Added at g3 (round 2's own tension, re-measured independently): one law conflict (R74 vs the round-1 A-power rule), delivered as a completed result (item 9).
...
| 9 | CONFLICT-R74-A-POWER | R74 vs the A-power rule (round-1 fix F2) | accepted (delivered configuration unchanged; declared g3) |
...
### 9. CONFLICT-R74-A-POWER — law conflict (R87) · status: accepted, delivered configuration unchanged (declared g3)

| | |
|---|---|
| **Laws** | R74 vs the A-power rule (round-1 fix F2) |
| **Condition** | Any wizard whose tier-0 cosmetic combo (head/cape/hat/wand) is not among the manifest's curated part tuples for its NEW live-tier power family, at any tier advance 0→1 through 0→6 |
| **The conflict** | R74 [LAW]: *"each combatant is the actual summoned wizard — its parts composited from its own traits."* Round-1 fix F2 made the rendered power family track the player's **live game tier**, matching the 2019 reference's own behaviour. Each power family lists only a curated subset of part tuples, so a wizard whose exact tier-0 tuple is not curated at its new power cannot literally keep "its own traits" there — `carryTraits()` (N-R74/F3b, wired into the read path at fix round F3) substitutes the deterministic nearest match instead. R74's "its own traits" and F2's "power tracks the live tier" cannot both hold literally for these wizards. |
| **Evidence** | **Independently re-measured, not copied from the reviewer's figures** — over `app/engine/wizardRig.js`'s `deriveTraits`/`deriveIdentity`: 400 synthetic ids (`char-0`..`char-399`) × 3 elements × tiers 1–6 = **7,200** tier-0-to-live-tier transitions, comparing `deriveTraits(id, el, 0)` against `deriveIdentity(id, el, tier).traits` on head/cape/hat/wand only. **Result:** 3,211/7,200 (**44.6%**) change no part; of the 3,989 that change anything, 3,970 (**55.1%** of the 7,200 total; 99.5% of the changed set) change **exactly one** part, 19 (**0.3%** of total; 0.5% of changed) change exactly two, and **none** change three or four. Per-part change rate (of the 7,200 total): **cape 0.0%**, **head 5.0%**, **wand 15.1%**, **hat 35.6%** (363/2,560/1,085/0 respectively; sums to 4,008 = 3,970×1 + 19×2, consistent by construction). Palette is keyed only on `characterId`, never tier — **100% stable** across every transition measured, matching §18's "a wizard's identity is its own colours filling that shape's slots." These numbers are close to but not identical to the reviewer's cited 45.1%/54.7%/0.2%/head 5.2%/wand 13.1%/hat 36.8% — expected, re-verifying independently rather than copying; the qualitative shape (~45% no-op, the overwhelming remainder a single-part swap, never more than two parts, cape always stable, hat the most volatile part) holds in both. Method: a short ad-hoc script over the cited pure/deterministic functions, not committed (out of this order's named scope), reproducible by anyone re-running the same id/element/tier sweep. |
| **Delivered configuration** | **Unchanged by this order.** F2's live-tracking power family wins; `carryTraits`' deterministic nearest-match (most tier-0 parts preserved, ties broken by the existing summon hash) is the closest available reconciliation under that constraint — only 0.3% of all transitions lose more than one part, and never more than two. The **closest law-preserving configuration** for R74 taken completely literally is different and **not shipped**: pin each wizard's power family at its own tier-0 power for cosmetic purposes only, which drives the churn to exactly zero but departs from the 2019 reference's own live-tracking behaviour F2 was built to match. Left as shipped, like `CONFLICT-R57-INTERNAL`; revisitable under an owner ruling (none requested yet). |
...
| O1…O10 | The ten open items, defaults + built alternatives (theme, price, legs, activation, commit 60s, lobby 30s [reversed from 60s at round 2, CB-BUILD-017 -- see `O6-LOBBY` below], fill-to-8, presets, 1170ms, temp 0.7) | C | A | acc | PRD §5 table; workbook rows W-30…W-39 (W-34 corrected at fix round f4 to match the shipped tunables.json) |
...
### Fix-round decisions (round 2 / g3 — CB-BUILD-009r + 014-022, AC2)

The reviewer's CRUCIAL finding at head `2610031`: round 2 moved these decisions and the graph recorded none of them (a grep for `009r`, `CB-BUILD-014`…`022`, `LINK_OPENED`, `lobbyHumanWait`, `R78a` turned up zero hits). Seven are recorded below as full decision nodes (AC2 evidence + rationale); four round-2 ids reviewed and found **not independently load-bearing** are noted with reasoning, not padded into full nodes (`NOTE-ROUND2-NON-LOAD-BEARING`, at the foot of this table) — see that node for why each of the four falls outside AC2's "moves the funnel, the economy, retention, or invariant compliance" test. `O6-LOBBY` (constraint spine, above) was corrected in place rather than duplicated here.

| Node | Decision | A | E | S | Evidence kind / source |
|---|---|---|---|---|---|
| D-R81-FULL-RANDOM-REVERT | CB-BUILD-009r: R81's missed-clock rule **reversed** by owner ruling (2026-09-15) from round-1's preserve-partial behaviour back to a uniform-random **five-move** auto-commit — the whole hand, including rounds already picked, flagged `hesitated`, excluded from staked-duel metrics (R42) | C | T | acc | `sync.js:autoCommitHesitated` (restored full-random signature, no picked-indices parameter); `commit-clock-autocommit.test.js` |
| D-SUMMON-CEREMONY | CB-BUILD-014: new LAW R74a — pressing SUMMON plays the full 2019 summon sequence for the chosen element and ends in the reveal of a random wizard of that element, its rig-derived parts/palette and name revealed together; seal-to-reveal duration instrumented (`SUMMON_CEREMONY_REVEALED`) | C | T | acc | `ui/components/summon/ceremony.js`; `ui/screens/summon.js`; `summon-ceremony.test.js` (6 tests) |
| C-R78A-SOUND | CB-BUILD-016: new LAW R78a — sound **on by default**, arms on the player's first tap, first heard at the summon ceremony, mute control visible wherever sound plays; f5 extended it (bed persists the whole bracket, round voices on the reference's own triggers, keydown also arms) | C | T | acc | `ui/components/battle/sound.js`; `overrides.js:defaultOverrides`; `sound-arming.test.js` |
| D-AC1-LINK-OPENED-CLOCK | CB-BUILD-017 fix round f4/Finding 2: AC1's link→first-seal clock now starts at a new boot-time `LINK_OPENED` ledger event (fired once, independent of screener pass/fail), not at `ACCOUNT_CREATED` — the old clock silently excluded the whole landing+screener interval, biasing the bar downward (repro: link t=0, screener pass t=40s, first seal t=100s used to report 60s, not 100s) | P | T | acc | `game.js:recordLinkOpened`; `retention.js:linkToFirstSeal`; `retention.test.js`; `link-to-first-seal.test.js` |
| D-IDENTITY-READ-TIME-CARRY | CB-BUILD-022 + fix round F3: identity derives at READ time from characterId/element/tier alone (never a recorded ledger field) **and** the wizard's own tier-0 cosmetic combo carries forward across tiers (N-R74/F3b) — F3 restored the carry after round 2's own initial cut silently dropped it (manager repro: 900/900 T0-vs-T1 differing) | P | T | acc | `wizardRig.js:deriveTraits/carryTraits/deriveIdentity/wizardIdentity`; `identity-read-time.test.js`; `rig-cosmetic-carry.test.js`; `rig-power-tier.test.js` |
| D-BUST-RETENTION-BRACKET | CB-BUILD-018: `bustToTopUp` (R49) now matches bracket-duel busts (nested `payload.combatants`), not only the flat practice-mode shape — the retention signal the deposit wall exists to serve was half-blind to real bracket busts; f4 advisory hardened the matcher against double-counting | P | T | acc | `retention.js` bust matcher; `retention.test.js` |
| NOTE-ROUND2-NON-LOAD-BEARING | CB-BUILD-015 (parity harness — AC1's own required verification tool, not itself a funnel/economy/retention/invariant decision), CB-BUILD-019 (tests reachable from the preview — a verifiability practice, zero runtime effect), CB-BUILD-020 (one spec file — repo hygiene, no behaviour change), CB-BUILD-021 (typography pardons audited — closes an audit gap under the *existing* R11/`C-R11-FIXED-FRAME`, doesn't move it) — reviewed against AC2's test and **not** given full nodes, reasoning recorded rather than the graph padded | P | A | acc | `cb-build-patch-log.md` CB-BUILD-015/019/020/021 entries |
...
## Edge summary (70 edges; full list in the JSON)
...
- **Conflict edges:** C-R71-NARRATOR and C-R12-THEME-BOUND → P-R71-NOUN-MAP (`conflicts-with`); C-R57-PVE → P-DRIFT-CONSTRUCTION (`conflicts-with`); C-R62-LADDER → P-T6-PAYOUT (`conflicts-with`); D-IDENTITY-READ-TIME-CARRY → CONFLICT-R74-A-POWER (`conflicts-with`, g3). The six conflicts declared as completed results after t2 (four at f3, one at f4, one at g3) live in the JSON's `conflicts` array directly (they need no open-question node — their configurations are delivered).
...
The t2 snapshot was static; **this f3 refresh is the rule operating**: the verification review and fix rounds f1/f2 ran, and the graph absorbed them — four review-found conflicts declared as completed results, nine fix-round decisions added as nodes, the citation counts corrected, the manifest-hypotheses section's obsoleted "built now" paper claim replaced with the verified build truth (its history preserved in the section note rather than deleted, per the supersede rule). **g3 keeps the rule operating a round later:** round 2 (CB-BUILD-009r + 014-022) ran, and the graph absorbed it too — seven more fix-round decisions added as nodes, one more conflict declared as a completed result (CONFLICT-R74-A-POWER, re-measured independently rather than copied), one stale node (O6-LOBBY) corrected in place, four non-load-bearing round-2 ids noted with reasoning instead of padded in, and a new test (`app/tests/decision-graph-freshness.test.js`) added so a future round's ids cannot go unmentioned again without turning the suite red. Going forward the delivered machine owns the live graph: door-race results move D-THEME-ADJ/D-THEME-FAR evidence from ASSUMED to TESTED (or supersede them per R22's band rule and brand-rejection rule); manifest runs move H-R26-* the same way; proposed nodes close on console rulings (A3). Any node marked `accepted` that a later phase contradicts is marked `superseded`, never deleted.
