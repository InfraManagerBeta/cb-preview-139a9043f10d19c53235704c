...
function reportMarkdown() {
  ...
  L.push('- **From in-repo reference assets:** the player configuration (parsed from `assets/cw/reference/Animation-index.js`), the portrait colour-slot system (parsed from `assets/cw/client-static/img/wizards/*.svg`), the O9 reveal cadence the step windows above are computed at (`app/data/tunables.json`), the canonical wizard\'s combo and fitted palette (round 1\'s `results/canonical-combo.json` and `results/check-a-gifs.json`), and the per-state reference clip lengths (pre-registered, and bound to `check-c-timing.mjs` by `app/tests/parity-preregistration.test.js`). All of these are digested by this run except the two `results/*.json` records — a record is not a source, and declaring one would make `npm run all` self-invalidating (check A rewrites its record before check E runs); the stated residual is an edit to that record itself, and check A\'s own inputs are gated.');
