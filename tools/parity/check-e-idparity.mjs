...
function reportMarkdown() {
  ...
  L.push(`- tree measured: commit \`${provenance.commit || 'unknown (not a git work tree)'}\`${provenance.branch ? ` (\`${provenance.branch}\`)` : ''}, working tree ${provenance.dirty ? `**DIRTY** — ${provenance.dirtyFiles.join(', ')}` : 'clean'}. Every number below is a claim about THAT tree and no other; \`results/check-e-idparity.json\` carries a content digest of the harness sources and of the engine modules the run measures, so a reader can tell whether it is still fresh.`);
  ...
  L.push(`- **Freshness:** these numbers describe commit \`${provenance.shortCommit || 'unknown'}\`${provenance.dirty ? ' plus uncommitted changes' : ''}. \`results/check-e-idparity.json\` records a sha256 of every harness source (asserted by \`app/tests/parity-harness.test.js\` — edit one without re-running and the suite fails) and of the engine modules this run measures (\`app/engine/wizardRig.js\`, \`rigRecolor.js\`, \`rigAssets.js\`, \`rigStage.js\`, the vendored player, the reference factory). The engine digests are RECORDED, not asserted: re-running needs bucket access and a native canvas build, which not every container has, so a reader compares them and re-runs deliberately (\`cd tools/parity && npm install && npm run check-e\`).`);
